...
-- Older installations may not have installed the reward activation migration.
-- Create its table with owner-only reads; financial writes are restricted below.
...
grant select on public.reward_activations to authenticated;
grant all on public.reward_activations to service_role;
...
on public.reward_activations for select to authenticated
